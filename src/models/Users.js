...
const findUser = async (email) => {
  try {
    const paras = [email];
    const query = `SELECT user_id, name, email FROM users.user WHERE email = ?`;
...
const findUsers = async () => {
  try {
    const query = `SELECT user_id, name, email FROM users.user`;
...
const getUserEmailAndPassword = async (email, password) => {
  try {
    // cpf = cpf.replace(/[.-]/g, "");
    const query = `SELECT user_id, name, email, password FROM users.user WHERE email = ?`;
    const params = [email];
